"""
TODO: Augmentace flip, ubrat rotaci, přiadat posunutí

TODO: A.RandomCrop(height=200, width=280, p=1.0)
"""
...
class GrayscaleWrapper(nn.Module):
    ...
    def forward(self, x):
        if x.shape[1] == 3:
            x = x.mean(dim=1, keepdim=True)  # Convert RGB to grayscale
...
class SSDFeatureExtractorResNet(nn.Module):
    def __init__(self):
        ...
        with torch.no_grad():
            resnet.conv1.weight[:] = original_conv1.weight.mean(dim=1, keepdim=True)


        layers = list(resnet.children())

        # Keep layers up to `layer3` (1024 channels, larger spatial size)
        # self.features = nn.Sequential(*layers[:8])  # Stops before layer4
        self.features = GrayscaleWrapper(nn.Sequential(*layers[:8]))

        # Extra SSD layers (first one also acts as a transition layer)
...
def Model(
    ...
    **kwargs: Any,
    ...
    backbone = SSDFeatureExtractorResNet()
    anchor_generator = DefaultBoxGenerator([
            [1.0, 0.5, 2.0],  # FM0: 15×20
            [1.0, 0.5, 2.0],  # FM1: 7×10
            [1.0, 0.5, 2.0],  # FM2: 3×5
            [1.0, 0.5, 2.0],  # FM3: 1×2
            [1.0, 0.5],  # FM4: 1×1 (no 2.0 here — square anchors suffice)
        ],
        scales=[0.07, 0.15, 0.33, 0.51, 0.69, 0.95],  # 6 scales → 5 feature maps
        steps=[32, 64, 160, 320, 608],
    )
    ...
    defaults = {
        ...
        "image_mean": None,
        "image_std": None,  # undo the 0-1 scaling of toTensor
    }
    kwargs: Any = {**defaults, **kwargs}
    model = SSD(backbone, anchor_generator, (608, 608), num_classes, **kwargs)
